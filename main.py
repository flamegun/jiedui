...
subject_num = int(input('请输入你要生成的题目个数：'))
scope = int(input('请输入你要生成四则运算的数的范围：'))
...
for i in range(0,subject_num):
    ...
    char_num = random.randint(1,3)#运算符的个数，不大于三个
    if char_num == 1:
    ...
    result = Fraction(result).limit_denominator()  #结果转为近似分数
    w = result.numerator
